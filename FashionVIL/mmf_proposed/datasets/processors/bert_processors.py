...
class MaskedTokenProcessor(BaseProcessor):
    ...
    def _convert_to_indices(
        ...
        assert len(input_ids) == self._max_seq_length, print(input_ids, text, len(input_ids), self._max_seq_length)
        assert len(input_mask) == self._max_seq_length, print(input_mask, text, len(input_mask), self._max_seq_length)
...
class BertTokenizer(MaskedTokenProcessor):
    ...
    def __call__(self, item: Dict[str, Any]):
        ...
        tokens_g = self.tokenize(text_g)
        tokens_l = self.tokenize(text_l)

        self._truncate_seq_pair(tokens_g, None, self._max_seq_length)
        self._truncate_seq_pair(tokens_l, None, self._max_seq_length)

        output_g = self._convert_to_indices(
            tokens_g, text_g, probability=self._probability
        )
        output_l = self._convert_to_indices(
            tokens_l, text_l, probability=self._probability
        )
        output = dict()
        for key, value in output_g.items():
            output[key + "_g"] = value
        for key, value in output_l.items():
            output[key + "_l"] = value

        output["text_g"] = output["tokens_g"]
        output["text_l"] = output["tokens_l"]
